...
if(!$mail->Send())
{
  ...
  echo '<meta http-equiv="refresh" content="0; url=http://lujq.me/register"/>';
}
}
// 参数说明(发送到, 邮件主题, 邮件内容,  用户名)
$content = '姓名：'.$_POST["name"].'<br>';
if (implode ($_POST["sex"])=="女") {
	$sex= '女';
}
else{
	$sex= '男';
...
$content .= '手机：'.$_POST["phone"].'<br>';
$content .= 'Q Q ：'.$_POST["qq"].'<br>';
$content .= '其他：'.$_POST["other"].'<br>';

smtp_mail('i@golmic.tk', '['.$_POST["name"]."] 申请注册", $content ,$_POST["name"]);
endif;
?>
</html>
